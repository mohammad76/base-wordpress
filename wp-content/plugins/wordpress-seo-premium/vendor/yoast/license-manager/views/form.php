	...
	<tbody>
		...
		<tr valign="top">
			...
			<td class="yoast-license-toggler">
				...
				<?php if( !$this->license_is_valid() ) { ?>
					<button name="<?php echo esc_attr( $action_name ); ?>" type="submit" disabled class="button button-secondary yoast-license-deactivate" value="deactivate"><?php echo esc_html_e( 'Deactivate License', $product->get_text_domain() ); ?></button> &nbsp;
		...
		<tr valign="top">
			...
			<td>
				<input name="<?php echo esc_attr( $key_name ); ?>" type="text" disabled class="regular-text textinput yoast-license-key-field <?php if( $obfuscate ) { ?>yoast-license-obfuscate<?php } ?>" value="<?php echo esc_attr( $visible_license_key ); ?>" placeholder="<?php echo esc_attr( sprintf( __( 'مجوز نسخه اورجینال این محصول به نام دیجیراتی ثبت و فعال شده است', $product->get_text_domain() ), $product->get_item_name() ) ); ?>" <?php if( $readonly ) { echo 'readonly="readonly"'; } ?> />
...
if( ! $embedded ) {
	...
	if( $readonly === true && $api_host_available['availability'] === false ) {
		submit_button();
	}
